...
//% weight=100 color=#0fbc11 icon=""
    ...
    export class Strip {
         ...
         */
        //% subcategory=ZIP_LEDs
        //% blockId="zip_set_strip_rainbow" block="%strip|show rainbow from %startHue|to %endHue" 
        //% weight=85 blockGap=8
        ...
        //% subcategory=ZIP_LEDs
        //% weight=84
        //% blockId=zip_show_bar_graph block="%strip|show bar graph of %value |up to %high" icon="\uf080" blockExternalInputs=true
        
        showBarGraph(value: number, high: number): void {
            if (high <= 0) {
                this.clear();
                this.setPixelColor(0, ZipLedColors.Yellow);
                this.show();
                return;
            }
            ...
            if (v == 0) {
                ...
                for (let i = 0; i < n; ++i) {
                    if (i <= v) {
                        let b = i * 255 / n1;
                        this.setPixelColor(i, kitronik.rgb(b, 0, 255 - b));
                    }
        ...
        /**
         * For ZIP LEDs with RGB+W LEDs, set the white LED brightness. This only works for RGB+W ZIP LEDs.
         * @param pixeloffset position of the LED in the strip
         * @param white brightness of the white LED
         */
        //% subcategory=ZIP_LEDs
        //% blockId="zip_set_pixel_white" block="%strip|set pixel white LED at %pixeloffset|to %white" 
        //% blockGap=8
        //% weight=80
         ...
         */
        //% subcategory=ZIP_LEDs
        //% blockId="zip_length" block="%strip|length" blockGap=8
        //% weight=60
        length() {
            return this._length;
        }
         ...
         **/
        //% subcategory=ZIP_LEDs
        //% blockId="zip_each_brightness" block="%strip|ease brightness" blockGap=8
        //% weight=58
        ...
        //% subcategory=ZIP_LEDs
        //% weight=89
        //% blockId="zip_range" block="%strip|range from %start|with %length|leds"
         ...
         */
        //% subcategory=ZIP_LEDs
        //% blockId="zip_shift" block="%strip|shift pixels by %offset" blockGap=8
        //% weight=40
         ...
         */
        //% subcategory=ZIP_LEDs
        //% blockId="zip_rotate" block="%strip|rotate pixels by %offset" blockGap=8
        //% weight=39
         ...
         */
        //% subcategory=ZIP_LEDs
        //% weight=9 blockId=zip_power block="%strip|power (mA)"
        power(): number {
            const stride = this._mode === ZipLedMode.RGBW ? 4 : 3;
            const end = this.start + this._length;
            let p = 0;
            ...
            return this.length() / 2 /* 0.5mA per ZIP LED */
     ...
     * @param pin the pin where the ZIP LED is connected.
     * @param numleds number of leds in the strip, eg: 24,30,60,64
     */
    //% subcategory=ZIP_LEDs
    //% blockId="zip_create" block="ZIP LED at pin %pin|with %numleds|leds as %mode"
    //% weight=90 blockGap=8
    
    //% trackArgs=0,2
    export function create(pin: DigitalPin, numleds: number, mode: ZipLedMode): Strip {
        let strip = new Strip();
        let stride = mode === ZipLedMode.RGBW ? 4 : 3;
    ...
    */
     ...
     */
    //% subcategory=ZIP_LEDs
    //% blockId=zipHSL block="hue %h|saturation %s|luminosity %l"
